...
At a backup checkpoint, connectors are drained and paused only while ClickHouse
...
potentially lagging Kafka Connect commit.
...
Kafka-to-ClickHouse transport retry from creating an extra target row. A
completed backup binds an immutable ClickHouse archive to the exact KeeperMap
checkpoint captured for that archive.
...
backup checks that the exact checkpoint offsets remain available immediately
...
- an automated restore or replay workflow;
...
Input topic retention must cover the intended lifetime of exact backup
checkpoints if another system will later consume those offsets.
